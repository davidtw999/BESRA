...
class B2M():
    ...
    def update(self):
        ...
        for n in range(self.N):
            gamma_update=np.exp(sp.digamma(self.Alpha[n]))/self.Beta[n]#this term is irrelevant to K so can be put here to save some computations.
            ...
            for k in range(self.K):
                ...
                for l in range(self.L):
                    phi_update=phi_update+self.Y[n,l]*(sp.digamma(self.A[k,l])-sp.digamma(self.A[k,l]+self.B[k,l]))
                #prevent overflow
    ...
    def predict_proba(self,test,res='simple',mode='ntest',sample=False,sampleSize=10,useAllK=True):
        ...
        if (mode=='test'):
            ...
            self.optPhi=optPhi
            MOR=MultiOutputRegressor(linear_model.BayesianRidge(fit_intercept=False)).fit(self.X,self.optPhi) #for testing
...
if __name__ == '__main__':
    ...
    Y = np.random.randint(2, size=(5, 3))
    print(X)
    print(Y)
    model1=B2M()
    model1.fit(X,Y,K=10,kernel_function=RBF(length_scale=1),K_threds=0.75)
    model1.learn(learnIter=15)
    prob, phi_new, mu,kappa = model1.predict_proba(X, res='full',mode='ntest',useAllK='True')
    print(prob.shape)
    print(phi_new.shape)
    print(mu.shape)
    print(kappa.shape)
