...
class ClassificationCollator(Collator):
    ...
    def __call__(self, batch):
        ...
        doc_token_len_bert = []
        doc_token_type_ids_bert = []
        doc_token_att_bert = []
        doc_token_ids_bert = []
        doc_token_max_len_bert = 1
        ...
        for _, value in enumerate(batch):
            ...
            for char_in_token in value[cDataset.DOC_CHAR_IN_TOKEN]:
                doc_char_in_token_max_len = max(doc_char_in_token_max_len,
                                                len(char_in_token))
            doc_token_max_len_bert = max(doc_token_max_len_bert,
                                    len(value[cDataset.DOC_INPUT_IDS_BERT]))
        ...
        for _, value in enumerate(batch):
            ...
            _append_vocab(value[cDataset.DOC_INPUT_IDS_BERT], doc_token_ids_bert, doc_token_max_len_bert)
            doc_token_len_bert.append(len(value[cDataset.DOC_INPUT_IDS_BERT]))
            _append_vocab(value[cDataset.DOC_TOKEN_ATT_BERT], doc_token_att_bert, doc_token_max_len_bert)
            _append_vocab(value[cDataset.DOC_TOKEN_TYPE_IDS_BERT], doc_token_type_ids_bert, doc_token_max_len_bert)
        ...
        batch_map = {
            ...
            cDataset.DOC_TOKEN_TYPE_IDS_BERT:
                torch.tensor(doc_token_type_ids_bert, dtype=torch.long),
            cDataset.DOC_TOKEN_ATT_BERT:
                torch.tensor(doc_token_att_bert, dtype=torch.long),
            cDataset.DOC_INPUT_IDS_BERT:
                torch.tensor(doc_token_ids_bert, dtype=torch.long)
